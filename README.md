...
一个基于Python和tkinter的GUI应用程序，用于打开各种格式的图像文件并提取像素点的颜色信息。

## 功能特性

- ✅ 支持多种图像格式：BMP, JPEG, PNG, GIF, TIFF, WebP, ICO
- ✅ 支持任意尺寸的图像文件
- ✅ 大图像智能处理和性能优化
- ✅ 图像缩放功能（25% - 400%）
- ✅ 鼠标点击获取像素点颜色信息
...
### 主要格式
- **BMP**: 位图格式，无压缩，最适合精确取色
- **JPEG/JPG**: 标准照片格式，应用最广泛
- **PNG**: 支持透明度的无损格式
- **GIF**: 动画和简单图像格式
- **TIFF/TIF**: 高质量专业图像格式
- **WebP**: 现代高效压缩格式
...
### 格式特性说明
- **无损格式** (BMP, PNG, TIFF): 颜色完全准确
- **有损格式** (JPEG): 可能有轻微颜色偏差
- **调色板格式** (GIF): 颜色数量有限但精确
- **透明度支持** (PNG, ICO): 自动处理透明背景
...
- **最小尺寸**: 1x1 像素
- **最大尺寸**: 理论上无限制
- **推荐尺寸**: 4096x4096 像素以下（最佳性能）

### 大图像处理
- 超过4096像素的图像会提示创建预览版本
- 预览版本用于显示，原始图像用于精确取色
- 自动内存管理，避免程序卡顿
- 支持高分辨率照片和设计文件
